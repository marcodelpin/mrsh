//! ~/.rsh/config parser — SSH-style config with Host blocks.
...
pub struct Config {
    ...
    /// Directory for session log files (default: ~/.rsh/logs).
...
impl Config {
    /// Returns the default config file path (~/.rsh/config).
    pub fn default_path() -> Option<PathBuf> {
        dirs::home_dir().map(|h| h.join(".rsh").join("config"))
    }
    ...
    /// Save config to the default path (~/.rsh/config).
...
"#;
    ...
    fn session_log_dir_default() {
        ...
        assert!(dir.ends_with(".rsh/logs") || dir.ends_with(".rsh\\logs"));
    }
    ...
    fn default_path_returns_some() {
        ...
        assert!(path.is_some());
        let p = path.unwrap();
        assert!(p.ends_with(".rsh/config") || p.ends_with(".rsh\\config"));
    }
}
